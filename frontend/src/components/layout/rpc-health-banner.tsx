...
export function RpcHealthBanner() {
  ...
  const usingProxy = !process.env.NEXT_PUBLIC_RPC_URL;
    ...
    <div className="flex items-center justify-center gap-2 bg-danger/15 px-4 py-2 text-center text-xs text-danger">
      ...
      <span>
        ...
        <code className="mx-1 rounded bg-black/20 px-1 py-0.5">
          {usingProxy ? "RPC_URL" : "NEXT_PUBLIC_RPC_URL"}
        </code>
        to a working BNB Smart Chain RPC {usingProxy && "on the server "}and restart.
      </span>
    </div>
  );
}
